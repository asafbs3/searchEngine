...
import com.handson.searchengine.AwsSqs.AwsProducer;
...
public class Crawler {
    ...
    public void crawl(String crawlId, CrawlerRequest crawlerRequest) throws InterruptedException, IOException {
        ...
        awsProducer.send(CrawlerRecord.of(crawlId, crawlerRequest));
    ...
    public void crawlOneRecord(String crawlId, CrawlerRecord rec) throws IOException, InterruptedException{
        ...
        logger.error("HTTP error fetching URL: " + rec.getUrl() + " Status: " + e.getStatusCode(), e);
        } catch (IOException e) {
        logger.error("Failed to fetch URL: " + rec.getUrl(), e);
        } catch (Exception e) {
        logger.error("Unexpected error occurred while crawling URL: " + rec.getUrl(), e);
        }
    }
...
//                producer.send(CrawlerRecord.of(rec).withUrl(url).withIncDistance()) ;
                awsProducer.send(CrawlerRecord.of(rec).withUrl(url).withIncDistance());
            }
        }
    }
